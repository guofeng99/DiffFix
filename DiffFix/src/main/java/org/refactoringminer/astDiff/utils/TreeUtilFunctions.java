...
public class TreeUtilFunctions {
	...
	public static Tree deepCopyWithMapPruning2(Tree tree, Map<Tree,Tree> cpyMap) {
		if (isFromType(tree, Constants.BLOCK))
			if (tree.getChildren().size() != 0) return null;
		Tree copy = makeDefaultTree(tree);
		cpyMap.put(copy,tree);
		cpyMap.put(tree,copy);
		if (isFromType(tree, Constants.ANONYMOUS_CLASS_DECLARATION)) return copy;
		for (Tree child : tree.getChildren()) {
			Tree childCopy = deepCopyWithMapPruning2(child,cpyMap);
			if (childCopy != null)
				copy.addChild(childCopy);
		}
		if (copy.getChildren().size() == 0) copy.setParent(tree.getParent());
	...
	public static  boolean isCall(String type){
		switch (type){
			case Constants.METHOD_INVOCATION:
			case Constants.SUPER_CONSTRUCTOR_INVOCATION:
			case Constants.CONSTRUCTOR_INVOCATION:
	...
	public static boolean isLeafStatement(String type){
		switch (type){
	...
	public static boolean isCompositeStatement(String type){
		switch (type){
			case Constants.BLOCK: // Composite
			case Constants.DO_STATEMENT: //composite
			case Constants.ENHANCED_FOR_STATEMENT: //composite
