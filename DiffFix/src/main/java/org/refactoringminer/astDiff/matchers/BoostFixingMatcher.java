...
public class BoostFixingMatcher extends BaseFixingMatcher {
    ...
    private final static Logger logger = LoggerFactory.getLogger(BoostFixingMatcher.class);
    ...
    public void match(Tree src, Tree dst, ExtendedMultiMappingStore mappingStore) {
        ...
        if(isDebug){
            ...
            pass_started = System.currentTimeMillis();
            matchByUnique(getPrunedMappedSrcs()); // around context only one missing
            pass_finished =  System.currentTimeMillis();
            logger.info("current pass: {}, execution: {} milliseconds",current_pass, pass_finished - pass_started);

            current_pass = "FixByChildren";
            pass_started = System.currentTimeMillis();
            fixByChildrenContext(getPrunedMappedSrcs()); // children context
            pass_finished =  System.currentTimeMillis();
            logger.info("current pass: {}, execution: {} milliseconds",current_pass, pass_finished - pass_started);

            current_pass = "FixByParent";
            pass_started = System.currentTimeMillis();
            fixByParentContext(getPrunedMappedSrcs()); // parent context
            pass_finished =  System.currentTimeMillis();
            logger.info("current pass: {}, execution: {} milliseconds",current_pass, pass_finished - pass_started);

//            current_pass = "fixUniqueAround";
//            pass_started = System.currentTimeMillis();
//            fixUniqueAround(getPrunedMappedSrcsWithIdentRoot());
//            pass_finished =  System.currentTimeMillis();
//            logger.info("current pass: {}, execution: {} milliseconds",current_pass, pass_finished - pass_started);

            current_pass = "FixByInner";
            pass_started = System.currentTimeMillis();
            fixByInnerContext(getPrunedMappedSrcs()); // fix calls; remove arbitrary mappings
            pass_finished =  System.currentTimeMillis();
            logger.info("current pass: {}, execution: {} milliseconds",current_pass, pass_finished - pass_started);

            current_pass = "FixByNearby";
            pass_started = System.currentTimeMillis();
            fixByNearbyContext(getPrunedMappedSrcsWithIdentRoot()); // nearby statement; identical statement; nearby condition
            pass_finished =  System.currentTimeMillis();
            logger.info("current pass: {}, execution: {} milliseconds",current_pass, pass_finished - pass_started);

//            current_pass = "matchByParentContextPre";
//            pass_started = System.currentTimeMillis();
//            matchByParentContextPre(getPrunedMappedSrcs());
//            pass_finished =  System.currentTimeMillis();
//            logger.info("current pass: {}, execution: {} milliseconds",current_pass, pass_finished - pass_started);
...
//            }
            ...
            pass_started = System.currentTimeMillis();
            matchByInnerContext(getPrunedMappedSrcs()); // inner call; this and null in return; conditional expression
            pass_finished =  System.currentTimeMillis();
            logger.info("current pass: {}, execution: {} milliseconds",current_pass, pass_finished - pass_started);

            current_pass = "MatchByNearby";
            pass_started = System.currentTimeMillis();
            matchByNearbyContext(getPrunedMappedSrcsWithIdentRoot()); // nearby statement;
            pass_finished =  System.currentTimeMillis();
            logger.info("current pass: {}, execution: {} milliseconds",current_pass, pass_finished - pass_started);

            current_pass = "MatchByChildren";
            pass_started = System.currentTimeMillis();
            matchByChildrenContext(getPrunedMappedSrcsWithIdentRoot()); // descendants context
            pass_finished =  System.currentTimeMillis();
            logger.info("current pass: {}, execution: {} milliseconds",current_pass, pass_finished - pass_started);

            current_pass = "MatchByParent";
            pass_started = System.currentTimeMillis();
            matchByParentContext(getPrunedMappedSrcs()); // parent context
            pass_finished =  System.currentTimeMillis();
            logger.info("current pass: {}, execution: {} milliseconds",current_pass, pass_finished - pass_started);
...
//            pass_started = System.currentTimeMillis();
//            fixByParentContextPost(getPrunedMappedSrcs()); // parent context
//            pass_finished =  System.currentTimeMillis();
//            logger.info("current pass: {}, execution: {} milliseconds",current_pass, pass_finished - pass_started);
        }
        else{
            ...
            current_pass = "FixByChildren";
            fixByChildrenContext(getPrunedMappedSrcs());
...
//            fixByParentContextPost(getPrunedMappedSrcs()); // parent context
    ...
    private Set<Tree> getPrunedMappedSrcs(){
        return Set.copyOf(prunedMappedSrcs);
    }

    private Set<Tree> getPrunedMappedSrcsWithIdentRoot() {return Set.copyOf(prunedMappedSrcsWithIdentRoot);}
